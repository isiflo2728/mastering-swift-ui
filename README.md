...
- **HotProspects** — A conference networking app for tracking people you've met. Scan QR codes to import contacts, manage prospects across Everyone / Contacted / Uncontacted tabs, swipe to mark contacted or delete, and schedule local notifications as follow-up reminders. Covers `TabView`, `@AppStorage`, `@Query` with `#Predicate` filtering, `SwiftData` for persistence, `CodeScanner` for QR scanning, `UNUserNotificationCenter` for local notifications, `CIFilter` / `CIContext` for QR code generation, `ShareLink`, and multi-select list editing. Lives on the `hot-prospects` branch.
- **FlashZilla** — A timed flashcard app with swipe-to-dismiss gesture interactions and full accessibility support. Build a deck of prompt/answer cards, swipe right to mark correct or left to mark wrong, and race against a 100-second countdown. Covers `DragGesture` with rotation and opacity feedback, sequenced and simultaneous gestures (`LongPressGesture`, `MagnifyGesture`, `RotateGesture`), `allowsHitTesting`, `Timer` publishers via `Combine`, `scenePhase` for background/foreground lifecycle, `UserDefaults` persistence with `Codable`, and accessibility environment values (`accessibilityDifferentiateWithoutColor`, `accessibilityVoiceOverEnabled`, `accessibilityReduceMotion`). Lives on the `FlashZilla` branch.
- **LayoutandGeometry** — A deep dive into SwiftUI's layout system and geometry tools. Covers the three-step parent/child size negotiation model, alignment guides with custom `VerticalAlignment` types, absolute positioning with `position()` vs relative offset with `offset()`, `GeometryReader` with global/local/named coordinate spaces, scroll-driven `rotation3DEffect` for a spinning helix effect, and a cover flow carousel built with `visualEffect`, `scrollTargetLayout`, and `scrollTargetBehavior(.viewAligned)`. Lives on the `layout-and-geometry` branch.
- **SnowSeeker** — An adaptive ski resort browser built for both iPhone and iPad. Browse resorts in a master/detail `NavigationSplitView`, search by name, mark favorites that persist in a shared `@Observable` model, and drill into a resort detail screen showing price, size, snow depth, elevation, and tappable facility icons with info alerts. Covers `NavigationSplitView` master/detail layout, JSON decoding via a generic `Bundle` extension into `Codable` models, `.searchable` filtering with `localizedStandardContains`, an `@Observable` `Favorites` store shared through the SwiftUI environment, country-flag list rows, and a detail layout that adapts to `horizontalSizeClass` and `dynamicTypeSize`. Lives on the `snowseeker` branch.
